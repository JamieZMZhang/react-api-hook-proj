...
export function useApi<TPayload = any, TData = TPayload>(
  params: UseApiParams<TPayload, TData>
) {
  ...
  const [request, setRequest] = React.useState({
    trigger: 0,
  } as ApiRequestParams & { trigger: number });
  ...
  React.useEffect(() => {
      ...
      try {
        const data =
          typeof params.onSuccess === 'function'
            ? params.onSuccess(response.data, response, request)
            : response.data;
        setData(data);
    ...
    const parallelKey = getKey(
      requestUrl,
      request.api?.parallelKey,
      params.parallelKey
    );
    let ongoingRequest: RequestSubject<AxiosResponse<any>> | undefined =
      parallelKey ? parallelStore[parallelKey] : undefined;
    ...
    if (!ongoingRequest) {
      ...
      const sendCall = () => {
        axios(axiosRequest)
          ...
          .catch((reason) => {
            if (Number(params.retry) > retryCountRef.current) {
              retryCountRef.current += 1;
              sendCall();
              return;
            }
            ongoingRequest!.error(reason);
          })
          .finally(() => {
  ...
  return [data, loadingState, trigger] as [
    typeof data,
    typeof loadingState,
    typeof trigger
  ];
